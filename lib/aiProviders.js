 ...
 * Supports: OpenAI, Azure OpenAI, Azure Foundry, Anthropic Claude, Google Gemini, Custom endpoints
 * 
 * Usage:
...
const PROVIDER_CONFIGS = {
  ...
  azure_foundry: {
    name: 'Azure Foundry',
    apiKey: () => process.env.AZURE_FOUNDRY_KEY || process.env.AZURE_AI_FOUNDRY_KEY || process.env.AZURE_INFERENCE_KEY,
    endpoint: () => process.env.AZURE_FOUNDRY_ENDPOINT || process.env.AZURE_AI_FOUNDRY_ENDPOINT,
    deployment: () => process.env.AZURE_FOUNDRY_DEPLOYMENT || process.env.AZURE_FOUNDRY_MODEL || 'gpt-4o-mini',
    embedDeployment: () => process.env.AZURE_FOUNDRY_EMBED_DEPLOYMENT || 'text-embedding-3-small',
    apiVersion: () => process.env.AZURE_FOUNDRY_API_VERSION || '2024-05-01-preview',
    authMode: () => (process.env.AZURE_FOUNDRY_AUTH_MODE || 'api-key').toLowerCase(),
    models: { chat: ['deployment'], embed: ['deployment'], image: [] }
  },
  anthropic: {
...
async function azureFoundryChat(messages, options = {}) {
  const config = PROVIDER_CONFIGS.azure_foundry;
  const apiKey = config.apiKey();
  const endpoint = config.endpoint();
  const deployment = options.deployment || config.deployment();
  const apiVersion = options.apiVersion || config.apiVersion();

  if (!apiKey || !endpoint) throw new Error('Azure Foundry not configured');
  ...
  if (!response.ok) {
    const error = await response.text();
    throw new Error(`Azure Foundry error: ${response.status} - ${error}`);
  }
...
async function azureFoundryEmbed(text, options = {}) {
  const config = PROVIDER_CONFIGS.azure_foundry;
  const apiKey = config.apiKey();
  const endpoint = config.endpoint();
  const deployment = options.deployment || config.embedDeployment();
  const apiVersion = options.apiVersion || config.apiVersion();
  ...
  if (!response.ok) {
    const error = await response.text();
    throw new Error(`Azure Foundry embedding error: ${response.status} - ${error}`);
  }
...
const PROVIDER_HANDLERS = {
  ...
  azure_foundry: { chat: azureFoundryChat, embed: azureFoundryEmbed, image: null },
...
export async function chat(provider, messages, options = {}) {
  ...
  if (options.useCache !== false && !options.stream && result && !result.then) {
    const cacheKey = getCacheKey(provider, 'chat', { messages, model: options.model });
    setCache(cacheKey, result);
  }

  if (options.deviceId && !options.stream) {
    trackUsage(options.deviceId, provider, 'chat', result.usage?.total_tokens || 0);
  }
...
export function getDefaultProvider() {
  ...
  // Fallback order
  const fallbackOrder = ['openai', 'azure', 'azure_foundry', 'anthropic', 'gemini', 'custom'];
